...
If you need to use more than one *TasksQueue*, you can store them into a *TasksQueuesContainer* instance. It has a simple `std::unordered_map<std::string, TasksQueue>` and a few convenience methods - `CreateQueue()`, `GetQueue()` and an `Update()` which will pass the update call to all contained queue's `Update()` methods.
