...
namespace TasksLib {
	...
	public:
		...
		void CheckStats(int added = -1, int completed = -1, int suspended = -1, int resumed = -1, int waiting = -1, int total = -1) {
			...
			if (suspended >= 0) {
				EXPECT_EQ(stats.suspended, suspended);
			}
			if (resumed >= 0) {
				EXPECT_EQ(stats.resumed, resumed);
			}
			if (waiting >= 0) {
				EXPECT_EQ(stats.waiting, waiting);
			}
			if (total >= 0) {
	...
	TEST_F(TasksQueueTest, AddsTask) {
		InitQueue();
		ASSERT_TRUE(queue.isInitialized());
